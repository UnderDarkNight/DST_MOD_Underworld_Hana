...
----- 设置海洋/虚空可用  allow_action_on_impassable
    function hana_com_point_and_target_spell_caster:SetAllowCanCastOnImpassable(flag)
...
---------------------------------------------------------------------------------------------------
----- 设置distance
    function hana_com_point_and_target_spell_caster:SetDistance(num)
        self.distance = num
    end
    function hana_com_point_and_target_spell_caster:GetDistance()
        return self.distance
    end
---------------------------------------------------------------------------------------------------
---- 设置优先级 priority
    function hana_com_point_and_target_spell_caster:SetPriority(num)
        self.priority = num
    end
    function hana_com_point_and_target_spell_caster:GetPriority()
        return self.priority or 0
    end
---------------------------------------------------------------------------------------------------
---- 自定义所有ACTION 参数
    function hana_com_point_and_target_spell_caster:SetActionParamFn(fn)
        self.action_param_fn = fn        
    end
    function hana_com_point_and_target_spell_caster:ActiveActionParam(theAction)
...
---------------------------------------------------------------------------------------------------
----- 设置str_index
    function hana_com_point_and_target_spell_caster:SetStrIndex(str_index)
        self.str_index = str_index
    end
---------------------------------------------------------------------------------------------------
----- 设置 str_index update fn
    function hana_com_point_and_target_spell_caster:SetTextUpdateFn(fn)
        if type(fn) == "function" then
            self.str_update_fn = fn
        end
    end
    function hana_com_point_and_target_spell_caster:ActiveTextUpdate(doer,target,pt)
...
-------------------------------------------------------------------------------------------------------- pre action
    ...
    function hana_com_point_and_target_spell_caster:DoPreAction(doer,target,pt)
...
---------------------------------------------------------------------------------------------------
------ 设置显示文本
    function hana_com_point_and_target_spell_caster:SetText(index,str)
        index = string.upper(index)
        STRINGS.ACTIONS.HANA_COM_POINT_AND_TARGET_SPELL_CASTER[index] = str
        self.str_index = index
    end
    function hana_com_point_and_target_spell_caster:GetTextIndex()
        return self.str_index
    end
---------------------------------------------------------------------------------------------------
------ 设置sg
    function hana_com_point_and_target_spell_caster:SetSGAction(sg)
        self.sg = sg
    end
    function hana_com_point_and_target_spell_caster:GetSGAction()
        return self.sg
    end
---------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------
