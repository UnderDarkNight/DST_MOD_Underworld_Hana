...
if TUNING.underworld_hana_DEBUGGING_MODE == true then
    -- modimport("key_modules_for_underworld_hana/03_AnimState_Hook/00_PlayAnim_PushAnim.lua")    --- 更新 PlayAnimation 和 PushAnimation 
    modimport("key_modules_for_underworld_hana/03_AnimState_Hook/01_OverrideSymbol.lua")    --- build fn    
end
---------------------------------------------------------------------------------------------------------------
------ 可以上线使用的模块
modimport("key_modules_for_underworld_hana/03_AnimState_Hook/00_PlayAnim_PushAnim.lua")    --- 更新 PlayAnimation 和 PushAnimation 
...
local function theAnimState_fn_Upgrade(theAnimState)
    for fn_name, fn in pairs(TUNING["underworld_hana.AnimStateFn"]) do
...
local function Hook_Player_AnimState(inst)
    ...
    if type(inst.AnimState) == "userdata" then            ----- 只能转变一次，重复的操作 会导致  __index 函数错误
        --------------------------------------------------------------------------------------------------------------------------------
            inst.__AnimState_userdata_underworld_hana = inst.AnimState      ----- 转移复制原有 userdata
                            ...
                            if _table and _table.inst and _table.name then
                                    ...
                                    if _G[_table.name][fn_name] then    ---- 从_G全局里得到原函数？？这句并不好理解。   ---- lua 会往_G 里自动挂载所有要运行的 userdata ？？
                                        ...
                                        local fn = function(temp_table,...)
                                            return _G[_table_name][fn_name](temp_table.inst.__AnimState_userdata_underworld_hana,...)
                                        end
    ...
    print("underworld_hana hook player AnimState finish")
end
...
local function Hook_Player_AnimState(inst)
    ...
    if type(inst.AnimState) == "userdata" then            ----- 只能转变一次，重复的操作 会导致  __index 函数错误
        inst.__AnimState_userdata_underworld_hana = inst.AnimState      ----- 转移复制原有 userdata
        inst.__AnimState_new_fns_underworld_hana = {}                ----- 新的函数在这里面， __index 里挂进去
            ...
            __index = function(_table,fn_name)
                        if _table and _table.inst and _table.name then
                            ------ 逻辑上复现棱镜模组的代码：
                                ...
                                if _table.inst.__AnimState_new_fns_underworld_hana[fn_name] then                       ----- 如果有hook过的函数，直接调用
                                    rawset(_table,fn_name,_table.inst.__AnimState_new_fns_underworld_hana[fn_name])
                                    return _table.inst.__AnimState_new_fns_underworld_hana[fn_name]
                                end
                                ...
                                if _G[_table.name][fn_name] then    ---- 从_G全局里得到原函数？？这句并不好理解。   ---- lua 会往_G 里自动挂载所有要运行的 userdata ？？
                                    ...
                                    _table.inst.__AnimState_new_fns_underworld_hana[fn_name] = fn      -- 缓存生成的镜像函数
                                    rawset(_table,fn_name,fn)
                                    return fn
                                end
